...
class CrudController extends BaseController {
    ...
    private function devolverValidation($validator, $modelo, Request $request, $registro = 0) {
        ...
        if ($request->ajax() || $tipoReturn == 'json') {
            $result = [
                'status' => 422,
                'statusText' => trans("sirgrimorum_cms::admin.messages.422"),
            ...
            if ($action == 'update') {
                ...
                $newaction = 'sirgrimorum_cms_modelo::edit';
            } else {
                $newaction = 'sirgrimorum_cms_modelos::create';
            }
            ...
            //return route('sirgrimorum_cms_modelo::' . $newaction, $extra) . $getReturn;
    ...
    private function devolver(Request $request, $config, $permiso, $objeto = 0) {
        ...
        $modeloM = ucfirst($modelo);
        $mensajes=[];
        if (is_array(trans("sirgrimorum_cms::admin.messages"))) {
            $mensajes = array_merge($mensajes, trans("sirgrimorum_cms::admin.messages"));
        }
        if (is_array(trans("sirgrimorum_cms::' . strtolower($modelo) . '.messages"))) {
            $mensajes = array_merge($mensajes, trans("sirgrimorum_cms::' . strtolower($modelo) . '.messages"));
        }
        if (!is_object($objeto)) {
            $registro = $objeto;
            if ($registro > 0) {
        ...
        if ($permiso) {
            ...
            if ($tipoReturn == 'json') {
                switch ($action) {
                    case "create":
                        $result = [
                            config("sirgrimorum_cms.status_messages_key") => $mensajes['na'],
                        ];
                        break;
                    case "index":
                    ...
                    case "destroy":
                        ...
                        $result = [
                            config("sirgrimorum_cms.status_messages_key") => str_replace(":modelName", $nombre, $mensajes[$action . "_success"])
                        ];
                        break;
                }
                ...
                switch ($action) {
                    case "index":
                    case "create":
                        $result = view('sirgrimorum_cms::admin.' . $action . "." . $tipoReturn, [
                            "modelo" => $modeloM,
                            "base_url" => route('sirgrimorum_cms_home', $localecode),
                            "plural" => $plural,
                            "config" => $config,
                        ]);
                        break;
                    ...
                    case "destroy":
                        ...
                        $result = redirect(route('sirgrimorum_cms_modelos::index', [
                                    'modelo' => $modelo,
                                    'localecode' => $localecode,
                                ]) . $getReturn)->with(config("sirgrimorum_cms.status_messages_key"), str_replace(":modelName", $nombre, $mensajes[$action . "_success"]))
                        ;
                        break;
                }
            }
            if ($request->ajax() || $tipoReturn == 'json') {
                $result = [
                    'status' => 200,
                    'statusText' => trans("sirgrimorum_cms::admin.messages.200"),
                    'result' => $result,
                ];
                if ($request->has('callback')) {
            ...
            switch ($action) {
                ...
                case "create":
                    $result = [
                        config("sirgrimorum_cms.error_messages_key") => $mensajes['permission'],
                        config("sirgrimorum_cms.login_redirect_key") => route("sirgrimorum_cms_modelos::" . $action, ['localecode' => $localecode, 'modelo' => $modelo])
                    ];
                    break;
                case "show":
                case "edit":
                    $result = [
                        config("sirgrimorum_cms.error_messages_key") => $mensajes['permission'],
                        config("sirgrimorum_cms.login_redirect_key") => route("sirgrimorum_cms_modelo::" . $action, ['localecode' => $localecode, 'modelo' => $modelo, 'registro' => $registro])
                    ];
                    break;
                case "store":
                case "destroy":
                    $result = [
                        config("sirgrimorum_cms.error_messages_key") => $mensajes['permission'],
                        config("sirgrimorum_cms.login_redirect_key") => route("sirgrimorum_cms_modelos::index", ['localecode' => $localecode, 'modelo' => $modelo])
                    ];
                    break;
                case "update":
                    $result = [
                        config("sirgrimorum_cms.error_messages_key") => $mensajes['permission'],
                        config("sirgrimorum_cms.login_redirect_key") => route("sirgrimorum_cms_modelo::edit", ['localecode' => $localecode, 'modelo' => $modelo, 'registro' => $registro])
                    ];
                    break;
            }
            if ($request->ajax() || $tipoReturn == 'json') {
                ...
                return redirect(config("sirgrimorum_cms.login_path"))->with($result);
            }
        }
    }
    ...
    private function validateModel($config, Request $request) {
        ...
        if (count($rules) > 0) {
            ...
            $error_messages = array_merge(trans("sirgrimorum_cms::admin.error_messages"), $error_messages);
