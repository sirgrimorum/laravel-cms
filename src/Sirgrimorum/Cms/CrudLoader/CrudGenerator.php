...
use Exception;
use Sirgrimorum\Cms\CrudLoader\CrudController;
...
class CrudGenerator {
    ...
    public static function getConfig($modelo) {
        if (!array_has(config("sirgrimorum_cms.admin_routes"), $modelo)) {
        ...
        $config = 'render';
        if (array_has(config("sirgrimorum_cms.admin_routes"), $modelo)) {
            $config = config(config("sirgrimorum_cms.admin_routes." . $modelo), 'render');
        }
        if (!is_array($config)) {
            $modeloClass = $config;
            if (!class_exists($modeloClass)) {
                ...
                if (!class_exists($modeloClass)) {
                    ...
                    if (!class_exists($modeloClass)) {
                        $modeloClass = "App\\" . $modelo;
                        if (!class_exists($modeloClass)) {
                            $modeloClass = "Sirgrimorum\\Cms\\Models\\" . $modeloM;
                            if (!class_exists($modeloClass)) {
                                $modeloClass = "Sirgrimorum\\Cms\\Models\\" . $modelo;
                                if (!class_exists($modeloClass)) {
            ...
            $config = [
                ...
                "botones" => trans("sirgrimorum_cms::admin.layout.labels.create"),
            ];
            $configCampos = [];
            $rules = [];
            if (\Lang::has('sirgrimorum_cms::' . $modelo)) {
                $transFile = 'sirgrimorum_cms::' . $modelo;
                        ...
                        switch ($datos['type']) {
                            ...
                            case 'timestamp':
                                $configCampos[$campo] = [
                                    ...
                                    'label' => $campo,
                                    'placeholder' => "",
                                    "format" => trans("sirgrimorum_cms::admin.formats." . $datos['type']),
                                ];
                                if ($datos['type'] == 'date') {
    ...
    public static function splitQueryNames($query) {
        ...
        if (($left = (stripos($query, "`"))) !== false) {
            while ($left !== false) {
                ...
                //echo "<pre>" . print_r(['prefix' => config("sirgrimorum_cms.trans_prefix"), 'lenprefix' => strlen(config("sirgrimorum_cms.trans_prefix")), 'left' => $left, 'rigth' => $right, 'piece' => $piece, 'lenpiece' => strlen($piece), 'csss' => $item], true) . "</pre>";
                $left = (stripos($query, "`"));
            }
        }
        return $resultado;
    ...
    public static function getTypeByName($name, $options) {
        if (!is_array($options)) {
            $options = config("sirgrimorum_cms.probable_" . $options);
        }
    ...
    public static function getNameAttribute($model) {
        ...
        $model->getKeyName();
        $compares = config("sirgrimorum_cms.probable_name");
        ...
        foreach ($array as $key => $item) {
            if (gettype($item) != "Closure Object") {
                if (is_array($item)) {
                    ...
                    if (str_contains($item, config("sirgrimorum_cms.trans_prefix"))) {
                        if (($left = (stripos($item, config("sirgrimorum_cms.trans_prefix")))) !== false) {
                            while ($left !== false) {
                                //echo "<pre>" . print_r($item, true) . "</pre>";
                                if (($right = stripos($item, '__', $left + strlen(config("sirgrimorum_cms.trans_prefix")))) === false) {
                                    $right = strlen($item);
                                }
                                $piece = trans(substr($item, $left + strlen(config("sirgrimorum_cms.trans_prefix")), $right - ($left + strlen(config("sirgrimorum_cms.trans_prefix")))));
                                if (is_string($piece)) {
                                    ...
                                    //echo "<pre>" . print_r(['prefix' => config("sirgrimorum_cms.trans_prefix"), 'lenprefix' => strlen(config("sirgrimorum_cms.trans_prefix")), 'left' => $left, 'rigth' => $right, 'piece' => $piece, 'lenpiece' => strlen($piece), 'csss' => $item], true) . "</pre>";
                                    $left = (stripos($item, config("sirgrimorum_cms.trans_prefix")));
                                } else {
    ...
    function create($config) {
        ...
        if ($config['url'] == "Sirgrimorum_CrudAdministrator") {
            $config['url'] = route("sirgrimorum_cms_modelo::store", ["localecode" => \App::getLocale(), "modelo" => strtolower(class_basename($config["modelo"]))]);
            $config['botones'] = trans("sirgrimorum_cms::article.labels.create");
        }
        $view = View::make('sirgrimorum_cms::crudgen.create', [
            'config' => $config, 
     ...
     */
    public function show($config, $id = null, $registro = null) {
        $config = CrudGenerator::translateConfig($config);
        if ($registro == null) {
        ...
        $view = View::make('sirgrimorum_cms::crudgen.show', array('config' => $config, 'registro' => $registro));
        return $view->render();
    }
     ...
     */
    public function edit($config, $id = null, $registro = null) {
        $config = CrudGenerator::translateConfig($config);
        if (isset($config['render'])) {
            foreach ($config['relaciones'] as $clave => $relacion) {
                $lista = array("-" => "-");
        ...
        if ($config['url'] == "Sirgrimorum_CrudAdministrator") {
            $config['url'] = route("sirgrimorum_cms_modelo::update", ["localecode" => \App::getLocale(), "modelo" => strtolower(class_basename($config["modelo"])), "registro" => $registro->id]);
            $config['botones'] = trans("sirgrimorum_cms::article.labels.edit");
        }
        $view = View::make('sirgrimorum_cms::crudgen.edit', [
            'config' => $config, 
            'registro' => $registro, 
            'tieneHtml' => $this->hasTipo($config, 'html'), 
            'tieneDate' => ($this->hasTipo($config, 'date') || $this->hasTipo($config, 'datetime')), 
    ...
    public function lists($config, $registros = null) {
        ...
        $view = View::make('sirgrimorum_cms::crudgen.list', array('config' => $config, 'registros' => $registros));
        return $view->render();
    }
     ...
     */
    public function lists_array($config, $registros = null) {
        $config = CrudGenerator::translateConfig($config);
        if ($registros == null) {
            $modeloM = ucfirst($config['modelo']);
            $registros = $modeloM::all();
     ...
     */
    public function registry_array($config, $registro = null) {
        $modeloM = ucfirst($config['modelo']);
        if ($registro == null) {
            $value = $modeloM::first();
        } else {
