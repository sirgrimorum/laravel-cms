...
if ($tieneHtml || $tieneDate || $tieneSlider || config("sirgrimorum_cms.own_bootstrap") || config("sirgrimorum_cms.own_jquery")) {
...
@include("sirgrimorum_cms::crudgen.templates." . $datos['tipo'])
@else
@include("sirgrimorum_cms::crudgen.templates.text")
@endif
@endforeach
...
<div class="form-group">
    <div class="{{ $config['class_offset'] }} {{ $config['class_divinput'] }}">
        {{ Form::submit(trans('sirgrimorum_cms::crud.create.titulo'), array('class' => $config['class_button'])) }}
    </div>
</div>
@endif
{{ Form::close() }}
